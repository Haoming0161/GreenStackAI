...
\paragraph{Model declaration for this submission.} The reported LLM numbers were generated with Tinker using \texttt{Qwen/Qwen3-30B-A3B-Instruct-2507} as the shared model across Single-Shot, One-Pass, and Agent settings, with 3 trials per LLM condition.
...
\paragraph{Baseline results.} Table~\ref{tab:baseline} reports four conditions using 3-trial evaluation for LLM-based methods. The static rule baseline applies deterministic rewrites (\texttt{join}, \texttt{@lru\_cache}, NumPy, \texttt{set()}, \texttt{heapq}) and returns the original when no rule fires; it remains high-correctness but can regress on specific tasks. Compared with single-shot prompting, one-pass profile guidance improves robustness but is sensitive to outliers (notably Multi-Pass Reduction). These outcomes support the benchmark's goal of evaluating both optimization quality and stability.

\begin{table}[h]
\centering
\caption{Baseline results on GreenPyBench (10 tasks; 3 trials for LLM systems). RT = runtime speedup, RAM = peak RAM reduction, both means over passing tasks.}
\label{tab:baseline}
\small
\begin{tabular}{lcccc}
\toprule
...
\begin{table}[h]
\centering
\caption{Per-task profiling for One-Pass Profile-Guided baseline (3-trial means).}
\label{tab:onepass_pertask}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Task} & \textbf{RT Speedup (\%)} & \textbf{VIS} \\
\midrule
Duplicate Detection   & 47.74 & 33.42 \\
Linear Search         & 97.16 & 68.01 \\
Loop Aggregation      & 62.08 & 43.45 \\
Top-K Selection       & 0.00  & 0.00 \\
Nested-Loop Join      & 95.44 & 66.81 \\
String Concatenation  & 96.53 & 67.77 \\
Redundant List        & 4.74  & 33.32 \\
Memoization           & 100.00 & 77.28 \\
Data Structure Choice & 99.16 & 69.41 \\
Multi-Pass Reduction  & -931.57 & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Per-task profiling for Critic--Refiner Agent (3-trial means).}
\label{tab:agent_pertask}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Task} & \textbf{RT Speedup (\%)} & \textbf{VIS} \\
\midrule
Duplicate Detection   & 99.89 & 69.92 \\
Linear Search         & 97.21 & 68.04 \\
Loop Aggregation      & 30.27 & 21.19 \\
Top-K Selection       & 22.27 & 45.36 \\
Nested-Loop Join      & 97.23 & 68.06 \\
String Concatenation  & 96.55 & 67.78 \\
Redundant List        & 4.60  & 33.22 \\
Memoization           & 100.00 & 77.28 \\
Data Structure Choice & 99.37 & 69.56 \\
Multi-Pass Reduction  & 0.00  & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
