...
\paragraph{Changelog.}
Final version adds confirmed per-task static rule results in Table~\ref{tab:pertask}, expands the two-measurement rollback guard discussion from Project Update 2~\cite{update2}, incorporates evaluation fairness clarifications from Project Update 3~\cite{update3}, and updates the implementation to support both direct Anthropic and Tinker-compatible LLM endpoints. Agent and LLM rows in Tables~\ref{tab:results}--\ref{tab:pertask} are pending API-limited evaluation completion.
